...
- Update check showing generic "Update check failed" error when release artifacts are not yet available -- now shows friendlier "No update info available yet" message for expected failures (404, network errors)
...
- Pre-existing Biome lint errors: non-null assertions in language queries and LanguageLevel view, explicit `any` in kanji test
