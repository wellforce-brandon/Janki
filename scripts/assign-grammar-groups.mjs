...
const PATTERN_RULES = [
	...
	// Particles (basic)
	{
		...
		keywords: ["particle", "direct object", "subject marker", "topic marker"],
	},
	...
	// Demonstratives & reference
	{
		...
		patterns: [/これ.*それ.*あれ/, /^Q〜でも$/, /^〜?など$/, /なんか/, /なんて/, /の?ことだから/],
	},
...
function extractExerciseTopic(primaryText) {
	...
	if (text.includes("verb") && text.includes("past")) return "Tae Kim: Past Tense";
	if (text.includes("verb") && text.includes("negative")) return "Tae Kim: Negative Verbs";
	if (text.includes("ru-verb") || text.includes("u-verb")) return "Tae Kim: Verb Basics";
	if (text.includes("verb")) return "Tae Kim: Verb Basics";
	if (text.includes("noun")) return "Tae Kim: Introduction to Particles";
	if (text.includes("explanatory")) return "Tae Kim: Noun-related Particles";
	if (text.includes("casual") || text.includes("polite")) return "Tae Kim: Using Adverbs and Gobi";
	if (text.includes("conjugat") || text.includes("past tense")) return "Tae Kim: Past Tense";
	if (
		text.includes("together with") ||
		text.includes("from something") ||
		text.includes("agreement") ||
		text.includes("informing")
	)
...
function extractSentenceGrammarGroup(item) {
	...
	if (
		explanation.includes("potential form") ||
		explanation.includes("able to speak") ||
		explanation.includes("able to do")
	)
	...
	if (
		explanation.includes("ください") ||
		explanation.includes("ちょうだい") ||
		explanation.includes("casual request")
	)
		return "Honorific & Polite Speech";
	if (
		explanation.includes("polite form") ||
		explanation.includes("humble") ||
		explanation.includes("honorif") ||
		explanation.includes("keigo")
	)
		return "Honorific & Polite Speech";
	if (
	...
	if (explanation.includes("because") || /\breason\b/.test(explanation)) return "Reason & Cause";
	if (explanation.includes("only ") || explanation.includes("しか"))
		return "Focus & Emphasis Particles";
	if (explanation.includes("might") || explanation.includes("かも")) return "Certainty & Judgment";
	...
	if (explanation.includes("比") || explanation.includes("compared")) return "Comparison & Degree";
	if (explanation.includes("giving") || explanation.includes("receiving"))
		return "Giving & Receiving";
	if (
		explanation.includes("particle") ||
		explanation.includes("topic marker") ||
		explanation.includes("subject marker")
	)
		return "Basic Particles";
	if (
		explanation.includes("conjugat") ||
		explanation.includes("verb stem") ||
		explanation.includes("polite form")
	)
		return "Verb Conjugation";
	if (explanation.includes("adjective")) return "Adjectives";
	if (explanation.includes("adverb")) return "Adverbs & Manner";
	if (explanation.includes("transitive") || explanation.includes("intransitive"))
		return "Causative & Passive Forms";
	if (
		explanation.includes("quotation") ||
		explanation.includes("quoting") ||
		explanation.includes("と言")
	)
	...
	if (text.includes("ください") || text.includes("下さい")) return "Honorific & Polite Speech";
	if (text.includes("座りな") || text.includes("食べな")) return "Honorific & Polite Speech";
	if (/申[もう]します/.test(text) || text.includes("お願いします"))
		return "Honorific & Polite Speech";
	if (text.includes("たら") || text.includes("ればいい")) return "Conditional Forms";
	...
	if (text.includes("じゃった") || text.includes("しちまった") || text.includes("ちゃった"))
		return "Te-form Patterns";
	if (text.includes("にくい") || text.includes("やすい")) return "Compound Verbs";
	if (text.includes("なった") || text.includes("にする") || text.includes("にします"))
		return "Result & Consequence";
	if (text.includes("たり")) return "Listing & Enumeration";
	if (text.includes("っけ")) return "Sentence-ending Particles";
	if (text.includes("のに")) return "Conjunctions & Contrast";
...
for (const item of grammar) {
	...
	const _isJlptDeck = decks.includes("Full_Japanese_Study_Deck_JLPT_N5N1_vocabkanji__more.apkg");
	...
	const isTaeKimGuide = decks.includes("Tae_Kims_Guide_to_Japanese_Grammar.apkg");
