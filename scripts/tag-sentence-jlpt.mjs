...
function buildVocabLookup(vocab) {
	...
	for (const item of vocab) {
		...
		if ((pos.includes("adjective") || text.endsWith("い")) && text.length >= 3) {
...
function analyzeSentence(plainText, vocabLookup, supplement) {
	...
	while (i < plainText.length) {
		...
		for (let len = Math.min(maxWordLen, plainText.length - i); len >= 2; len--) {
			const substr = plainText.slice(i, i + len);

			const jlpt = vocabLookup.get(substr) || supplement.get(substr);
			if (jlpt) {
		...
		if (!matched) {
			...
			if (isKanji(ch)) {
				...
				if (kanjiJlpt && JLPT_ORDER[kanjiJlpt] <= JLPT_ORDER.N3) {
					highestJlpt = highestJlpt ? jlptHigher(highestJlpt, kanjiJlpt) : kanjiJlpt;
...
if (existsSync(JMDICT_PATH)) {
	...
	console.log("JMDict not found, using vocab-only lookup (less comprehensive)");
}
