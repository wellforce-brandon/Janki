...
interface Props {
	/** For card-based items: raw field values */
	fields: Record<string, string>;
	/** Anki template string (card-based) or pre-rendered HTML (builtin) */
	frontTemplate: string;
	backTemplate: string;
	/** If true, templates are already rendered HTML, skip field substitution */
	preRendered?: boolean;
	css?: string | null;
	flipped: boolean;
	onflip: () => void;
...
let { fields, frontTemplate, backTemplate, preRendered = false, css = null, flipped, onflip }: Props = $props();

let frontHtml = $derived(
	sanitizeCardHtml(preRendered ? frontTemplate : renderCardContent(fields, frontTemplate)),
);
let backHtml = $derived(
	sanitizeCardHtml(preRendered ? backTemplate : renderCardContent(fields, backTemplate)),
);
</script>
