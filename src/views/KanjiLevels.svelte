...
import { getAllLevelProgress, getUserLevel, type LevelProgress } from "$lib/db/queries/kanji";
...
function getProgressForLevel(level: number): LevelProgress | undefined {
	...
	load().then(() => {
		if (initialTier > 0 && initialTier < TIERS.length) {
			requestAnimationFrame(() => {
				const el = document.getElementById(`tier-section-${initialTier}`);
...
<div class="space-y-6">
	<h2 class="text-2xl font-bold" tabindex="-1">Levels</h2>
	...
	{:else}
		<div class="space-y-8">
			{#each TIERS as tier, tierIdx}
				<section id="tier-section-{tierIdx}" class="space-y-3">
					...
					<div class="grid grid-cols-5 gap-2 sm:grid-cols-10">
						{#each { length: tier.end - tier.start + 1 } as _, i}
							...
							<button
								...
								onclick={() => navigate("kanji-level", { level: String(level) })}
